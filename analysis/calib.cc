...
int main(int argc, char* argv[]) {
      ...
      for (int i = 0; i < sipmData.size(); i++) {
        ...
        for(auto TmpItr = timeItr.begin(); TmpItr != timeItr.end(); ++TmpItr) {
          ...
          if(RecoInterface::IsCerenkov(sipmData[i].x, sipmData[i].y)) {
            if(timeData.first.first<36.15){ // tikim added this line for X-talk correction
  ...
  //-----------------------------------------------
  // I added to find the minimum value for Xtalk correction.
  double curMin = 999999;
  int idxMin = 0;
  for (int i = 1; i < 601; i++ ) {
    double tmpMin = tCtime->GetBinContent(i);
    if(0.1*i+10>30){
      if(0.1*i+10<40){
        if ( tmpMin <= curMin ) {
          curMin = tmpMin;
  ...
  std::cout << "CThres : " << idxMin << std::endl;

  std::cout << tCtime->GetBinCenter(idxMin-1)  << " " << tCtime->GetBinCenter(idxMin)  << " " << tCtime->GetBinCenter(idxMin+1)  << std::endl;
  std::cout << tCtime->GetBinContent(idxMin-1) << " " << tCtime->GetBinContent(idxMin) << " " << tCtime->GetBinContent(idxMin+1) << std::endl;
